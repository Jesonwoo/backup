...
DWORD Initialize()
{
	...
	wsprintf(DirverPath, _T("%s\\%s"), dir, gDriverFileName);

	if(IsFileExist(DirverPath) == FALSE)
	{
        //std::string driverPath = ws2s(DirverPath);
		return OLS_DLL_DRIVER_NOT_FOUND;
	}

	if(IsOnNetworkDrive(DirverPath) == TRUE)
	{
	...
	if(gIsNT)
	{
		...
		ManageDriver(OLS_DRIVER_ID, DirverPath, OLS_DRIVER_REMOVE);
		if(!ManageDriver(OLS_DRIVER_ID, DirverPath, OLS_DRIVER_INSTALL))
		{
			ManageDriver(OLS_DRIVER_ID, DirverPath, OLS_DRIVER_REMOVE);
